...
void RungeKuttaFourth::integrate(const std::vector<Particles *> &particles,
  ...
  int i = 0;
  for (const auto &p : particles) {
   p->position() = p->position() + (h * particles_1[i]->velocity()) / 2;
  ...
  i = 0;
  for (const auto &p : particles) {
   p->position() = p->position() + (h * particles_2[i]->velocity()) / 2;
  ...
  i = 0;
  for (const auto &p : particles) {
   p->position() = p->position() + h * particles_3[i]->velocity();
  ...
  for (const auto &p : particles) {
    ...
    x_k1 = h * particles_1[i]->position();
    v_k1 = h * particles_1[i]->velocity();
    
    x_k2 = h * particles_2[i]->position();
    v_k2 = h * particles_2[i]->velocity();
    
    x_k3 = h * particles_3[i]->position();
    v_k3 = h * particles_3[i]->velocity();
     
    x_k4 = h * particles_4[i]->position();
    v_k4 = h * particles_4[i]->velocity();
